...
| `${WORK_DIR}/tuning-package_<YYYYMMDD_HHMMSS>.tar.gz` | 调优脚本打包 |
...
| 调优方向 | 指南目录 | 中间态输出 |
|--------|--------|-----------|
| Docker算力统筹调优 | `references/docker-coordination-burst-tuning/` | `docker-coordination-burst-tuning.md` |
| numa并行感知调度调优 | `references/numa-sched-tuning/` | `numa-sched-tuning.md` |
| 窃取任务调度调优 | `references/stealtask-tuning/` | `stealtask-tuning.md` |
| 网卡多路径调优 | `references/multi-net-path-tuning/` | `multi-net-path-tuning.md` |
| 分域调度调优 | `references/soft-domain-tuning/` | `soft-domain-tuning.md` |
| 动态 SMT 调优 | `references/dynamic-smt-tuning/` | `dynamic-smt-tuning.md` |