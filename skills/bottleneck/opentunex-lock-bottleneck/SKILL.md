...
- **lock_stat not available**: Requires root and kernel CONFIG_LOCK_STAT=y; if unavailable, fall back to /proc/softirqs and perf-based analysis.
- **perf_event_paranoid restriction**: If perf_event_paranoid > 0, suggest `echo 0 > /proc/sys/kernel/perf_event_paranoid` or running as root; fall back to wchan and /proc analysis.
- **Target process exited**: If --pid is specified but the process has exited, fall back to system-wide lock analysis and note the process absence.
- **perf record fails**: Check disk space and permissions; if perf fails, use /proc/locks, /proc/softirqs, and ps wchan as fallback data sources.
