...
| 步骤 | 操作 | 产出 |
|------|------|------|
| 1 | 执行 `../../scripts/opentunex-stealtask-analysis/preanalysis.sh ${DATA_DIR} ${DATA_DIR}/opentunex-stealtask-analysis_collect` | `preanalysis.json` |
...
> **优先级**：本技能提供 `../../scripts/opentunex-stealtask-analysis/preanalysis.sh` 脚本对原始采集数据进行预处理。优先执行脚本生成 `preanalysis.json`，然后基于 JSON 进行分析。逐文件读取原始数据仅作为降级路径。

### 优先路径：预分析 JSON（推荐）

1. 执行预处理脚本生成 JSON：
   ```bash
   bash ../../scripts/opentunex-stealtask-analysis/preanalysis.sh ${DATA_DIR} ${DATA_DIR}/opentunex-stealtask-analysis_collect
   ```
...
```bash
# 使用调优脚本（推荐）
bash ../../scripts/opentunex-stealtask-analysis/stealtask_tune.sh check    # 环境检查
bash ../../scripts/opentunex-stealtask-analysis/stealtask_tune.sh apply    # 宿主机模式
...
```bash
bash ../../scripts/opentunex-stealtask-analysis/stealtask_tune.sh rollback
...
```bash
# 宿主机模式
bash ../../scripts/opentunex-stealtask-analysis/stealtask_tune.sh apply

# 容器模式（需先在 grub.cfg 添加 group_steal 并重启）
bash ../../scripts/opentunex-stealtask-analysis/stealtask_tune.sh apply --container <cgroup_path>
```
