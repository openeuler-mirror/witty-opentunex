...
| 步骤 | 操作 | 产出 |
|------|------|------|
| 1 | 执行 `../../scripts/opentunex-numa-sched-analysis/preanalysis.sh ${DATA_DIR} ${DATA_DIR}/opentunex-numa-sched-analysis_collect` | `preanalysis.json` |
...
> **优先级**：本技能提供 `../../scripts/opentunex-numa-sched-analysis/preanalysis.sh` 脚本对原始采集数据进行预处理。优先执行脚本生成 `preanalysis.json`，然后基于 JSON 进行分析。逐文件读取原始数据仅作为降级路径。
...
1. 执行预处理脚本生成 JSON：
   ```bash
   bash ../../scripts/opentunex-numa-sched-analysis/preanalysis.sh ${DATA_DIR} ${DATA_DIR}/opentunex-numa-sched-analysis_collect
   ```
...
```bash
# 使用调优脚本（推荐）
bash ../../scripts/opentunex-numa-sched-analysis/numa_sched_tune.sh check    # 环境检查
bash ../../scripts/opentunex-numa-sched-analysis/numa_sched_tune.sh apply    # 启用 PARAL + sched_util_low_pct=100
...
```bash
bash ../../scripts/opentunex-numa-sched-analysis/numa_sched_tune.sh rollback
...
```bash
bash ../../scripts/opentunex-numa-sched-analysis/numa_sched_tune.sh apply
```
