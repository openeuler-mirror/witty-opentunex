...
> **优先路径**：若使用 `preanalysis.json`，直接检查 `pmu_path_applicable` 字段：`true` → 进入 NA1-NA3 判定；`false` → 降级到路径 B。
...
| 优先级 | 条件 | 结论 | 原因 |
|-----|------|------|------|
| NA1 | OPS_PER_SEC ≤ OPS_THRESHOLD (200万) | 收益有限 | HHA 每秒操作量 {X} < 2,000,000，未达到远程瓶颈判定门槛 |
| NA2 | OPS_PER_SEC > OPS_THRESHOLD 且 REMOTE_RATIO ≤ REMOTE_THRESHOLD (5%) | 收益有限 | 操作速率达标 ({X}/s) 但远程访问占比 {X}% ≤ 5%，NUMA 本地性良好 |
| NA3 | OPS_PER_SEC > OPS_THRESHOLD 且 REMOTE_RATIO > REMOTE_THRESHOLD (5%) | **适用** | 操作速率 {X}/s > 2,000,000 且远程访问占比 {X}% > 5%，NUMA 内存访问存在瓶颈 |
