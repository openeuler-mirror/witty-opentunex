...
# Process IO config (if PID provided)
if [ -n "$TARGET_PID" ] && [ -d "/proc/$TARGET_PID" ]; then
    ...
    # cgroup IO throttling for this process
...
for dev in $(lsblk -d -n -o NAME | grep -E '^vd|^sd|^nvme' | head -5); do
    ...
    # 1) Try sysfs msi_irqs directory (NVMe and some SCSI)
    ...
    # 2) Walk up sysfs to PCI device, check its irq file
    if [ $found -eq 0 ]; then
        pci_bdf=""
    ...
    # 3) Fallback: match block device to /proc/interrupts by driver name
    if [ $found -eq 0 ]; then
