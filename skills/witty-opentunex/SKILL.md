...
**执行前必须先判断**：上下文中是否指定了已采集的数据目录？

- 是：检查上下文指定的数据目录是否存在？（检查时**必须**以上下文的完整相对路径或者绝对路径为准，**禁止**试图尝试其它类似路径，**禁止**检查其它任何目录或者文件寻找疑似“数据目录”的路径）
  - 是：基于上下文指定的数据目录进入**阶段2**
  - 否：直接退出整个流程，提示上下文指定的数据目录不存在，要求重新指定正确的数据目录或者自动采集
- 否：继续按下列要求执行**阶段1**
