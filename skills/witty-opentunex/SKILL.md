...
- 进入瓶颈分析阶段后，agent 必须**直接读取**数据采集阶段产出的文件作为分析输入，**禁止**调用任何采集脚本（如 `server_data_collector.sh` / `collect_*.sh`），也**禁止**通过 `top` / `iostat` / `sar` / `vmstat` / `mpstat` / `pidstat` / `perf` 等命令临时读取系统数据
...
| 阶段 | 子技能 | 必须产出的文件 | 路径 |
|-----|-------|--------------|------|
| 阶段1 - 数据采集 | `opentunex-data-collection` | 9 个基础维度的采集报告（cpu / mem / io / net / process / system / kernel / container / pmu） | 轻量编排：`${WORK_DIR}/<item>-collection_collect/<item>-collection_report.txt`；一键采集：`${WORK_DIR}/<采集项名>.txt`（路径以脚本 `-o` 为准） |
...
```bash
# 阶段1 → 阶段2 前：校验采集文件就位（轻量编排模式为例）
for item in cpu mem io net process system kernel container pmu; do
    f="${WORK_DIR}/${item}-collection_collect/${item}-collection_report.txt"
    [ -s "$f" ] || echo "MISSING_OR_EMPTY: $f"
done

# 阶段2 → 阶段3 前：校验分析报告就位
[ -s "${WORK_DIR}/analysis/opentunex-top-down-bottleneck_collect/result.md" ] \
    || echo "MISSING_OR_EMPTY: top-down-bottleneck report"
[ -s "${WORK_DIR}/analysis/opentunex-scenario-bottleneck_collect/result.md" ] \
    || echo "MISSING_OR_EMPTY: scenario merged report"

# 阶段3 完成时：校验调优产物就位
[ -s "${WORK_DIR}/tuning/tuning-report.md" ] \
    || echo "MISSING_OR_EMPTY: tuning report"
[ -s "${WORK_DIR}/tuning-package_"*.tar.gz ] \
    || echo "MISSING: tuning package tarball"
```
...
**执行流程**：
1. **数据采集阶段**：调用 `opentunex-data-collection` 技能，采集系统指标，产出采集文件（默认位于 `${WORK_DIR}/`）
   - **如果是远端目标**：先调用 `opentunex-remote-execution` 建立 SSH 连接，SCP 上传采集脚本到远端 `${WORK_DIR}/collect/scripts/`，再 SSH 执行脚本
...
1. 先通过 SCP 将 `opentunex-data-collection/scripts/` 下的脚本文件上传到远端 `${WORK_DIR}/collect/scripts/`
2. 再通过 SSH 执行上传后的脚本
3. **禁止**将脚本内容拆解为单条命令（mpstat/iostat/vmstat 等）直接 SSH 执行
